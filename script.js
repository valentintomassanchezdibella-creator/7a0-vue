...
const jugadoresR = document.getElementById("jugadores-random");
...
lista.forEach(j => {
    ...
    jugadorDiv.classList.add("jugadorDiv")

    jugadorDiv.innerHTML = `
            <h3>${j.nombre}</h3>
            <span>${j.media}</span><br>
            <span>${j.posicion}</span>`

    jugadoresR.appendChild(jugadorDiv)

    jugadorDiv.addEventListener("click" , () =>{
        jugadorDiv.classList.toggle("active")
    })
});
